...
//空间对齐
 ...
 public:
  ...
  //创建内存集(覆盖给定的保留区域，允许最多分裂为“max_covered_regions”) 
  ...
  //此方法控制收集器如何满足对内存块的请求。 如果自适应大小策略确定花费了过多的时间进行收集并导致返回NULL，
  //则gc_time_limit_was_exceeded"将被设置为true。 如果不返回NULL，则“gc_time_limit_was_exceeded”具有未定义的含义。 
...
// All of hotspot's current collectors are subtypes of this class. 
// 当前所有的hotspot收集器都是这个类的子类
//Currently, these collectors all use the same gen[0],
//当前这些收集器都使用相同的gen[0]
// but have different gen[1] types. If we add another subtype
//但是有不同的gen[1]类型。如果我们添加另一个子类型的收集器策略，这个类将会分解成自己的文件
