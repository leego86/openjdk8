...
void GenCollectedHeap::process_roots(bool activate_scope,
  ...
  //_process_strong_tasks的_n_terminate应该在一个不在GC worker中运行的方法中设置流。 
  //否则，当任务在另一个GC worker中执行时，GC worker可能试图改变终止条件 
  ...
  // 线程框架中欧给你中包含的一些CLDs，应该被认为是强引用
  // Don't process them if they will be processed during the ClassLoaderDataGraph phase.
  //如果它们将在ClassLoaderDataGraph阶段处理，不要处理它们
  CLDClosure* roots_from_clds_p = (strong_cld_closure != weak_cld_closure) ? strong_cld_closure : NULL;
  // Only process code roots from thread stacks if we aren't visiting the entire CodeCache anyway
  //只有当我们不访问整个CodeCache时，才可以处理线程栈中的根
  ...
  //所有的现场执行下面的代码。来自StringTable的特定buckets块是单独任务
  ...
  if (!_process_strong_tasks->is_task_claimed(GCH_PS_CodeCache_oops_do)) {
    ...
    if (so & SO_AllCodeCache) {
      ...
      //我们扫描整个代码缓存，因为CodeCache::do_unload没有被调用 
